...
class Airi {
    ...
    private static $original_option_name = 'airi_options';
    ...
    private function __construct() {
        ...
        wp_cache_add_non_persistent_groups( 'airi' );
        ...
        $this->settings       = new Airi_Setting();
        $this->images         = new Airi_Images();
        $this->layout         = new Airi_Layout();
        $this->options        = new Airi_Options();
        $this->favorite       = new Airi_FavoritePosts();

        new Airi_Init();
        new Airi_Blog();
        new Airi_Widget_CssClass();
    ...
    public function build_crumbs(){
        ...
        if( $this->settings()->get('google_rich_snippets', 'no') == 'yes' ){
            if(function_exists('wc') && is_object(wc()->structured_data)){
                add_action('airi/action/breadcrumbs/before_render_html', array(wc()->structured_data, 'generate_breadcrumblist_data'), 10);
            }
        }
        $this->breadcrumbs = new Airi_Breadcrumbs( $args );
    }

    public function set_is_updating() {
        if ( ! self::$is_updating && $_GET && isset( $_GET['airi_update'] ) && '1' == $_GET['airi_update'] ) {
    ...
    public function set_current_context(){
        ...
        $coreTypeAllows = array(
            ...
            'la_block' => array(
                'single'    => 'is_single_la_block',
                'archive'   => 'is_archive_la_block',
                'tax'       => 'is_tax_la_block'
            ),
            'la_testimonial' => array(
                'single'    => 'is_single_la_testimonial',
                'archive'   => 'is_archive_la_testimonial',
                'tax'       => 'is_tax_la_testimonial'
            ),
            ...
            'ld_release' => array(
                'single'    => 'is_single_ld_release',
                'archive'   => 'is_archive_ld_release',
                'tax'       => 'is_tax_ld_release',
                'taxonomy'  => array(
                    'ld_band' => 'is_tax_ld_band',
                    'ld_label' => 'is_tax_ld_label'
                )
            ),
            'lpm_playlist' => array(
                'single'    => 'is_single_lpm_playlist',
                'archive'   => 'is_archive_lpm_playlist',
                'tax'       => 'is_tax_lpm_playlist',
            ),
            'la_video' => array(
                'single'    => 'is_single_la_video',
                'archive'   => 'is_archive_la_video',
                'tax'       => 'is_tax_la_video',
                'taxonomy'  => array(
                    'video_type' => 'is_tax_la_video_type',
                    'video_tag' => 'is_tax_la_video_tag'
                )
            )
        );
    ...
    public static function multilingual_options() {
        // Set the self::$lang.
        if ( ! in_array( Airi_Multilingual::get_active_language(), array( '', 'en', 'all' ) ) ) {
            self::$lang = '_' . Airi_Multilingual::get_active_language();
        }
