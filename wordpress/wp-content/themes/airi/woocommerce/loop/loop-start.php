...
$active_shop_masonry = Airi()->settings()->get('active_shop_masonry', 'off');
$shop_masonry_column_type = Airi()->settings()->get('shop_masonry_column_type', 'default');
$woocommerce_shop_masonry_columns = Airi()->settings()->get('woocommerce_shop_masonry_columns');

$product_masonry_container_width = Airi()->settings()->get('product_masonry_container_width', 1170);
$product_masonry_item_width = Airi()->settings()->get('product_masonry_item_width', 270);
$product_masonry_item_height = Airi()->settings()->get('product_masonry_item_height', 450);
$woocommerce_shop_masonry_custom_columns = Airi()->settings()->get('woocommerce_shop_masonry_custom_columns');
$shop_masonry_item_setting = Airi()->settings()->get('shop_masonry_item_setting');


$woocommerce_pagination_type = Airi()->settings()->get('woocommerce_pagination_type', 'pagination');
...
$column_tmp = Airi()->settings()->get('woocommerce_shop_page_columns');

if($is_main_loop && airi_string_to_bool($active_shop_masonry)){
    $column_tmp = $woocommerce_shop_masonry_columns;
}

?>
<?php
$view_mode = Airi()->settings()->get('shop_catalog_display_type', 'grid');
...
if($is_main_loop && airi_string_to_bool($active_shop_masonry) && $shop_masonry_column_type == 'custom'){
...
$view_mode = apply_filters('airi/filter/catalog_view_mode', $view_mode);
...
if($is_main_loop && airi_string_to_bool($active_shop_masonry)){
    airi_set_wc_loop_prop('prods_masonry', true);
    ...
    if( $shop_masonry_column_type != 'custom' ){
        ...
        $loopCssClass[] = airi_render_grid_css_class_from_columns($columns);
    }
    else{
        ...
        if(!empty($shop_masonry_item_setting) && is_array($shop_masonry_item_setting)){
            foreach($shop_masonry_item_setting as $k => $size){
                ...
                if(!empty($size['image_size'])){
                    $__new_item_sizes[$k]['s'] = Airi_Helper::get_image_size_from_string($size['image_size']);
                }
            }
        }
        airi_set_wc_loop_prop('item_sizes', $__new_item_sizes);
        $masonry_component_type[] = 'AdvancedMasonry';
    }
    airi_set_wc_loop_prop('image_size', Airi_Helper::get_image_size_from_string(Airi()->settings()->get('product_masonry_image_size', 'shop_catalog')));
    ?>
<?php
}
else{
