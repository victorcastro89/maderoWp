...
<div class="product--large-image clearfix<?php echo ( ($product_design == 3 || $product_design == 4) ? ' force-disable-slider-script' : '') ?>">
	<div data-product_id="<?php the_ID()?>" class="<?php echo esc_attr( implode( ' ', array_map( 'sanitize_html_class', $wrapper_classes ) ) ); ?>" data-columns="<?php echo esc_attr( $columns ); ?>" style="opacity: 0; transition: opacity .25s ease-in-out;">
		<div class="woocommerce-product-gallery__actions">
			<?php
			$video_link = Airi()->settings()->get_post_meta($product->get_id(), 'product_video_url');
			if(!empty($video_link)){
				printf('<a class="video-link-popup la-popup" href="%s" rel="nofollow"><span><i class="dl-icon-format-video"></i></span></a>', esc_url($video_link));
			}
			?>
		</div>
		<figure class="woocommerce-product-gallery__wrapper">
			...
			if(function_exists('wc_get_gallery_image_html')){
				...
				if ( has_post_thumbnail() ) {
					...
					$html .= sprintf( '<img src="%s" alt="%s" class="wp-post-image" />', esc_url( wc_placeholder_img_src() ), esc_html_x( 'Awaiting product image', 'front-view', 'airi' ) );
					$html .= '</div>';
				}
			}
